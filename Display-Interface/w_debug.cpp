...
//    connect(PWR_btnGroup, SIGNAL(buttonClicked(int)), this, SLOT(PWRButtonJudge(int)));
    ...
    ui->text_Unbalance->setText(set_Test_File->value("System/Unbalance", "1").toString());
    ...
    if (set_Test_File->value("stagnate/text", 0).toInt() == 2) {  //  防呆
        ui->checkBox_stagnate->setChecked(true);
    } else {
        //
    }
...
void w_Debug::on_Button_Conf_clicked()
{
    ...
    if (ui->checkBox_stagnate->checkState() == 2) {
        set_Test_File->setValue("stagnate/text", "2");
    } else {
        set_Test_File->setValue("stagnate/text", "0");
    }
