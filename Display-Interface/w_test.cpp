    ...
    ui(new Ui::w_Test)
{
    ...
    ui->testTab->setColumnWidth(0, 35); // -测试表格的宽度调整
    ui->testTab->setColumnWidth(1, 77);
    ui->testTab->setColumnWidth(2, 253);
  ...
  //          ui->testTab, SLOT(resizeRowsToContents()) );
    ...
    QPushButton *button_dump = new QPushButton;
    ...
    connect(button_dump, SIGNAL(clicked()), this, SLOT(join_autodump()));
    ...
    IR_ACW_Error.clear();
    IR_ACW_Error.append("1");        // -1
    IR_ACW_Error.append("2");        // -2
    IR_ACW_Error.append("3");        // -3
    IR_ACW_Error.append("4");        // -4
    IR_ACW_Error.append("LEAK");     // -5
    IR_ACW_Error.append("OVER-PWR"); // -6
    IR_ACW_Error.append("DCPT");     // -7
    IR_ACW_Error.append(">20mA");    // -8
    ...
    Hall_A_ing = false;
    Ready_Stop_NOLOAD = false; NOload_Up_Finsh = false;
    Ready_Stop_BEMF   = false; BEMF_Up_Finsh = false;
    Ready_Stop_LOAD   = false; LOAD_Up_Finsh = false;

    PG_Reset_OK = false;

    for (i = 0; i < 370; i++) {
       Bemf_Update.append("0");
       Load_Update.append("0");
    }
...
void w_Test::join_updateTime()
{
    int j;
//    if (Hall_A_ing) {
//        qDebug() << "Test-----------PG Hall A B C ";
//        frame.can_id = 0x2C;  frame.can_dlc = 0x03; // Test-----------PG Hall A
//        frame.data[0] = 0x09; // 读取频率　占空比
//        frame.data[1] = 0x01;
//        frame.data[2] = 0x00;
...
//    }
    ...
    if (Temp_In) { //-温度过高
        if (count%2 == 0) {
            ...
            ui->label_5->setText(QString("机箱内部温度过高-%1，建议停止使用").arg(In_Temp));
            ui->label_5->show();
        } else {
            ui->label_5->setStyleSheet("background-color:red");
            ui->label_5->setText(QString("机箱内部温度过高-%1，建议停止使用").arg(In_Temp));
    ...
    int Current_Row;
    //   qDebug() << "PG_Result_List---------------->>>>>>>>>>>>>>>"<<PG_Result_List.size();
...
void w_Test::Board_ACW_Data(QStringList ACW_Data_List)
{
    ...
    str = ACW_Data_List.at(0) + (QString::number(ACW_Data_List.at(1).toDouble(), 'f', 2)) + "mA";
    ...
    default:
        ...
        strResult.append(IR_ACW_Error.at(ACW_Data_List.at(2).toInt()));
        str = "NG";
        Test.NG_Item = 4;
        break;
    }
...
void w_Test::Board_IR_Data(QStringList IR_Data_List)
{
    ...
    default:
        strResult.removeLast();
        strResult.append(IR_ACW_Error.at(IR_Data_List.at(2).toInt()));
        str = "NG"; Test.NG_Item = 3;
        break;
    }
    strJudge.append(str); str.clear();
...
void w_Test::Board_IMP_Data(QStringList IMP_Data_List)
{
    ...
    if (Data_Complete) {
        ...
        for (int i = 0; i < IMP_Row; i++) {
            if (abs(IMP_Data_List.at(i).toInt()) <= strITT.at\
                    (IMP_Current_Testing_Item*820 + 4 + i).toInt()) {
                str = "OK";
            } else {
                str = "NG";
                Test.NG_Item = 6;
        ...
        if ((Ini_imp_min) && (str  ==  "OK")) {
            ...
            for (int i = 0; i < 2; i++) {
                if ((IMP_Data_List.at(i).toInt()) >= strITT.at \
                        (IMP_Current_Testing_Item*820 + 17 + i).toInt()) {
                    str = "OK";
                } else {
                    str = "NG";
                    Test.NG_Item = 6;
...
//    {
//        double  INDL_Balance_All,INDL_Balance_Average,INDL_Balance_Value,INDL_Balance_Set;
...
//    }
    if ((strINDL.at(1) != "0.0") && (INDL_Total_Number >= 3)) {
        ...
        double Res_Balance_difference = 0;
        double Compare_One, Compare_Two;
        double Compare_Finsh;
    ...
    if ((strINDL.at(7).toDouble() != 0.0) && (INDL_Total_Number >= 3)) {
        double  Q_Balance_All, Q_Balance_Average, Q_Balance_Value, Q_Balance_Set;
...
void w_Test::Board_DLR_Data(QStringList DLR_Data_List)
{
    ...
    if ((DLR_Data_List.size() == 1) && (DLR_Data_List.at(0) == QString(tr("Error")))) {
        Test_WMessage(QString(tr("温度传感器有问题,请关掉温度补偿")));
        return;
    }
    ...
    if (DCR_Total_Number == DCR_Test_Record_Number) { // -判断电阻是否测试完成
        if (DLR_UpDown.at(20).toDouble() != 0.0) {   // -计算电阻不平衡度
...
{
    double  Res_Balance_All, Res_Balance_Average, Res_Balance_Value, Res_Balance_Set;
    double  Res_Balance_difference; // -电阻不平衡度两两数值差
    QString Res_Balance_List;
    bool    Res_Balance_Judge = 0;
    int     i, j;
    Res_Balance_Set = DLR_UpDown.at(20).toDouble();
    if ((Ini_NoBalance == 1)&&(DCR_Total_Number >= 3)) { // -普通电阻三个测试不平衡度
        ...
        for (i = strResult.size() - dlr_conut; i < strResult.size(); i++) {
            ...
            Res_Balance_List += (QString::number(Res_Balance_Value, 'f', 2)+"% ");
        ...
        Test.Current_Row++; // -行数增加
    } else if ((Ini_NoBalance == 2) && (DCR_Total_Number >= 2)) { // -两两相减的电阻差
        ...
        Test.Current_Row++; // -行数增加
    } else if ((Ini_NoBalance == 3) && (DCR_Total_Number >= 3)) { // -两两相减的电阻差
        Res_Balance_difference = 0;
        double Compare_One, Compare_Two;
        double Compare_Finsh;
        for (j = strResult.size(); j > strResult.size() - dlr_conut; j--) {
...
void w_Test::join_test_start_two(QStringList value) {
    QStringList list;
    list.append(QString::number(PG_Fre));
    list.append(QString::number(PG_Bb));
    list.append(QString::number(PG_High));
    list.append(QString::number(PG_Low));
    list.append(value);
...
//                QString::number(PG_High) + "V," + QString::number(PG_Low) + "V";
    ...
    H_Lever = Updatedata_List.at(261 + 20*FG_Test_Count).toDouble() * 15.68 / 4096;
    L_Lever = Updatedata_List.at(260 + 20*FG_Test_Count).toDouble() * 15.68 / 4096;

    FG_list.clear();
    FG_list.append(QString::number((H_Lever),'f',2));
    FG_list.append(QString::number((L_Lever),'f',2));
    FG_list.append(QString::number((Updatedata_List.at(350 + 4*FG_Test_Count).toDouble()/1000),'f',1));
    FG_list.append(QString::number((Updatedata_List.at(351 + 4*FG_Test_Count).toDouble()/1000),'f',1));
...
void w_Test::Board_NOLOAD_Data(QStringList Updatedata_List) {
    ...
    Updatedata_List.replace(3,QString::number((int(PG_Fre))/strNOLOAD.at(13).toInt()));

    Noload_list.clear();
    if (Station_Chose == 0x13) {
        I_get = Updatedata_List.at(0).toDouble() + strNOLOAD.at(15).toDouble();
    } else {
        I_get = Updatedata_List.at(0).toDouble() + strNOLOAD.at(16).toDouble();
    }
    ...
    P_get = I_get * Updatedata_List.at(1).toDouble();
    Noload_list.append(QString::number(I_get, 'f', 2));
    Noload_list.append(QString::number(Updatedata_List.at(1).toDouble(), 'f', 1));
    Noload_list.append(QString::number(P_get, 'f', 1));
    Noload_list.append(QString::number(Updatedata_List.at(3).toInt()));

    if (Updatedata_List.at(4).toInt() > 2) {
        Updatedata_List.replace(4, QString::number(0));
    }
    ...
    if (strNOLOAD.at(11).toInt() != 0) {
        str += ",";
        str += FG_direction.at(Updatedata_List.at(4).toInt());
    }
    strResult.append(str);
    if (!NoLoadFinsh) {
        qDebug() << "return";
    ...
    if ((strNOLOAD.at(11).toInt() != 0) && (str == "OK")) {
        if (Updatedata_List.at(4).toInt() != strNOLOAD.at(11).toInt()) {
            str = "NG";
            Test.NG_Item = 13;
        } else {
...
void w_Test::Board_LOAD_Data(QStringList Updatedata_List) {
    ...
    str = (QString::number(Updatedata_List.at(0).toDouble(), 'f', 2)) + "mA," +
        //    (QString::number(Updatedata_List.at(1).toDouble(), 'f', 0)) + "V," + \
...
//        }
    ...
    if (Test.Next_Finsh_Count != Test.Finshed_Count) { // -测试单个项目和总项目是否完成
        ...
        case 7: // -功率
            ...
            break;
        case 8: // -电感
            PC_Test_Result_ItemData(QString::number(INDL_Total_Number), "INDL");
            break;
        case 9: // -堵转
            //
            break;
...
void w_Test::IO_Pumb_Start_timeout() {
    ...
    IO_Test_Readying = false;
    Test_WMessage(QString(tr("  气缸动作超时  ")));
}
...
void w_Test::Test_Start()
{
    ...
    case 5:
        ...
        break;
    case 6:
        qDebug() << "Test ITT" << IMP_Total_Number;
...
void w_Test::Test_Process() {
 ...
 // Modification_Soft_Askoll(Station);
    ...
    Ready_Stop_NOLOAD = false;
    Ready_Stop_BEMF   = false;
    Ready_Stop_LOAD   = false;
    NOload_Up_Finsh = false;
    LOAD_Up_Finsh = false;
...
void w_Test::join_test_start(int Model, int Station)
{
    ...
    if ((!(Test_GND_Volt/1000)) && (Ini_NineBoard == 2)) {
        Test_WMessage(QString("    系统接地不良--%1     \n 请先检查地线再进行测试 ").arg \
                      (Test_GND_Volt%1000));
        return;
    }
    qDebug() << "1";
...
void w_Test::Can_Start_PWR(int str)
{
    ...
    int i, time, fre, grade, position_grade;
    for (i = 0; i < 36; i++) {
        usleep(1000);
        qApp->processEvents();
    }
    ...
    if ((Ini_Motor == QString(tr("PG"))) && ((strPWR.at(5 + position_grade).toInt() == 2))) {
        ...
        frame.data[2] = (time-2)/256;
        frame.data[3] = (time-2)%256;
    ...
    if (Ini_Power_Chose == 0) {      // -无电源
        ...
        frame.data[4] = ((1 << 4) | (fre) | (strPWR.at(25).toInt()/256));
        frame.data[5] = strPWR.at(25).toInt()%256;
    } else if (Ini_Power_Chose == 2) {  // -外部电源板
        frame.data[4] = (2 << 4);
        frame.data[5] = (strPWR.at(30).toInt());
    } else if (Ini_Power_Chose == 3)  { // -变压器方案
        frame.data[4] = ((3 << 4) | (strPWR.at(25).toInt()/256));
        frame.data[5] = strPWR.at(25).toInt()%256;
    } else {
        frame.data[4] = 0x00;
        frame.data[5] = 0x00;
...
void w_Test::Can_Start_BLOCK()
{
    ...
    if (Ini_Power_Chose == 0) {     //-无电源
        ...
        frame.data[3] = ((1 << 4) | (fre) | (strBLOCK.at(200).toInt()/256));
        frame.data[4] = strBLOCK.at(200).toInt()%256;
    } else if (Ini_Power_Chose == 2) { // -外部电源
        frame.data[3] = (2 << 4);
        frame.data[4] = (strBLOCK.at(207).toInt());
    } else if (Ini_Power_Chose == 3) { // -变压器方案
        frame.data[3] = ((3 << 4) | (strBLOCK.at(200).toInt()/256));
        frame.data[4] = strBLOCK.at(200).toInt()%256;
    } else {
        frame.data[3] = 0x00;
        frame.data[4] = 0x00;
...
void w_Test::Can_Start_LVS()
{
    ...
    frame.data[0] = 0x01; // -设置功率参数
    if (Station_Chose == 0x13) {
        frame.data[1] = 0x01;    // -档位
    } else {
        frame.data[1] = 0x01 << 4; // -档位
    }
    ...
    if (Ini_Power_Chose == 0) {   // -无电源
        ...
        frame.data[4] = ((1 << 4) | (fre) | (strLVS.at(0).toInt()/256));
        frame.data[5] = strLVS.at(0).toInt()%256;
    } else if (Ini_Power_Chose == 2) { // -外部电源
        frame.data[4] = (2 << 4);
        frame.data[5] = (strLVS.at(2).toInt());
    } else if (Ini_Power_Chose == 3) { // -变压器方案
        frame.data[4] = ((3 << 4) | (strLVS.at(0).toInt()/256));
        frame.data[5] = strLVS.at(0).toInt()%256;
    } else {
...
void w_Test::join_test_start_one()
{
    qDebug() << "join_test_start_one()";
    Hall_A_ing = true;
    emit WriteMessage(6000, 0x72, QString("").toUtf8());
    if (Conf_Item.at(Test.Finshed_Count).toInt() == 13) {
        qDebug() << "Wait noload ...";
//        join_test_noload();
        QTimer *Button_Start = new QTimer(this);
        connect(Button_Start, SIGNAL(timeout()), this, SLOT(join_test_noload())); //
        Button_Start->setSingleShot(TRUE);
        Button_Start->start(1200);  // 空载时间 1200ms
    } else if (Conf_Item.at(Test.Finshed_Count).toInt() == 12) {
        qDebug() << "Wait load ...";
//        join_test_lold();
        QTimer *Button_Start = new QTimer(this);
        connect(Button_Start, SIGNAL(timeout()), this, SLOT(join_test_lold())); //
        Button_Start->setSingleShot(TRUE);
        Button_Start->start(1200);  // 负载时间 1200ms
    } else {
        //
    }
}

void w_Test::join_test_noload() {
    int i;
    int j;

    qDebug() << "join_test_noload()";
    if (Ready_Stop_NOLOAD) {
        Ready_Stop_NOLOAD = false;
        Can_Stop(0x2C);
        return;
    }
    NOload_Up_Finsh = true;
    qDebug() << "Send PG------Hall-A";
    ...
    frame.can_id = 0x2C;  frame.can_dlc = 0x04;  // Hall-All // 卧龙 2017-10-10 // 10-27
    frame.data[0] = 0x0c; // 读取测试波形
    frame.data[1] = FG_Conf_Item.at(0).toInt()%1000; // (Hall-E Hall-D Hall-C Hall-B    Hall-A EMF-C EMF-B EMF-A)
    frame.data[2] = 144; // 400 % 256
    frame.data[3] = 1;   // 400 / 256
    canSend(frame, wTest_Surface);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 10ms
        usleep(1000);
    }

    frame.can_id = 0x2C;  frame.can_dlc = 0x03; // 读取高低电平 // 卧龙 2017-10-10
    frame.data[0] = 0x0A; // 读取高低电平
    frame.data[1] = 0x00;
    frame.data[2] = 0x00;
    canSend(frame, 88);
    for (j = 0; j < 10; j++) { // 下发参数延时等待 50ms
        usleep(1000);
    }

    frame.can_id = 0x2C;  frame.can_dlc = 0x02; // 测试交/直流电参命令
    frame.data[0] = 0x11; // 测试交/直流电参命令
    frame.data[1] = 0x01;
    canSend(frame, wTest_Surface);
}

void w_Test::join_test_lold() {
    int j;
    qDebug() << "join_test_lold()";
    if (Ready_Stop_LOAD) {
        Ready_Stop_LOAD = false;
        Can_Stop(0x2C);
        return;
    ...
    frame.can_id = 0x2C;  frame.can_dlc = 0x04;  // Hall-All // 卧龙
    frame.data[0] = 0x0c;
    frame.data[1] = FG_Conf_Item.at(1).toInt()%1000;
    frame.data[2] = 144; // 400 % 256
    frame.data[3] = 1;   // 400 / 256
    canSend(frame, wTest_Surface);
    for (j = 0; j < 50; j++) { // 下发参数延时等待 50ms
        usleep(1000);
    }
    frame.can_id = 0x2C;  frame.can_dlc = 0x03; // 频率 // 卧龙
    frame.data[0] = 0x09; // 读取频率　占空比
    frame.data[1] = 0x01;
    frame.data[2] = 0x00;
    canSend(frame, 88);
    ...
    frame.can_id = 0x2C;  frame.can_dlc = 0x03; // 读取高低电平 // 卧龙
    frame.data[0] = 0x0A;
    frame.data[1] = 0x00;
    frame.data[2] = 0x00;
    canSend(frame, 88);
    ...
    frame.can_id = 0x2C;  frame.can_dlc = 0x02; // 设置采集边沿个数命令
    frame.data[0] = 0x11; // 设置采集边沿个数命令
    frame.data[1] = 0x01;
    canSend(frame, wTest_Surface);
}
...
void w_Test::join_test_bemf() {
    int j;
    if (Ready_Stop_BEMF) {
        qDebug() << "Jump BEMF Test----->Stop";
    ...
    BEMF_Up_Finsh = true;
    qDebug() << "Send BEMF------Phase-Differece  Fre  Hall-All  H-L";
    frame.can_id = 0x2C;  frame.can_dlc = 0x01; // 相位差
    frame.data[0] = 0x0E;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 30; j++) { // 下发参数延时等待 30ms
        usleep(1000);
    }
    frame.can_id = 0x2C;  frame.can_dlc = 0x04;  // Hall-All
    frame.data[0] = 0x0c; // 波形
    frame.data[1] = FG_Conf_Item.at(2).toInt()%1000; qDebug() << "frame.data[1]" << frame.data[1];
    frame.data[2] = 144; // 400 % 256
    frame.data[3] = 1;   // 400 / 256
    canSend(frame, wTest_Surface);
    for (j = 0; j < 50; j++) { // 下发参数延时等待 60ms
        usleep(1000);
    }
    frame.can_id = 0x2C;  frame.can_dlc = 0x03; // 频率
    frame.data[0] = 0x09;
    frame.data[1] = 0x01;
    frame.data[2] = 0x00;
    canSend(frame, 88);
    ...
    frame.can_id = 0x2C;  frame.can_dlc = 0x03; // 读取高低电平
    frame.data[0] = 0x0A;
    frame.data[1] = 0x00;
    frame.data[2] = 0x00;
    canSend(frame, 88);
...
void w_Test::Can_Start_LOAD() {
    ...
    frame.data[7] = 0x00;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 100; j++) { // 下发参数延时等待 100ms
        usleep(1000);
    }
...
//    join_updatedata(QStringList("")); // 德科
//    return;
//    if (!PG_Reset_OK) {
//        join_updatedata(QStringList(""));
//        return;
...
void w_Test::Can_Start_BEMF() {
    ...
    frame.data[2] = 0x02; // 01 vcc    02 vsp    03 vm
    frame.data[3] = 0x01; // 01 缓升    02 缓降
    frame.data[4] = 1; // *4095/16.8
    frame.data[5] = 0; // *4095/16.8
    frame.data[6] = 1;
    frame.data[7] = 0;
    canSend(frame, wTest_Surface);
    for (j = 0; j < 100; j++) { // 下发参数延时等待 100ms
    ...
    QTimer *Button_Start = new QTimer(this);
    connect(Button_Start, SIGNAL(timeout()), this, SLOT(join_test_bemf())); //
    Button_Start->setSingleShot(TRUE);
    Button_Start->start(2*1000);
    BEMF_Issue_Ing = false;
}
...
/******************************************************************************
  ...
  * brief:      测试界面进行警告提示
******************************************************************************/
void w_Test::Test_WMessage(QString Waring_Text)
{
...
void w_Test::Test_PWR_Widget(double volt, int value)
{
    if (Conf_Item[Test.Finshed_Count].toInt() == 1) { // -测试假功率
        ...
        if ((False_Test) && (value == 2) && (volt < 5)) { // -反电动势大于5V
            ...
            join_test_stop(3);
            Test_WMessage(tr("  测试失败，反电动势过高--%1 V  ").arg(volt));
        } else {
            //
        }
    } else {
...
void w_Test::Pub_Main_to_test(QStringList list, QString str, int value, int flag)
{
        ...
        if (Test.Finshed_Count < Conf_Item.size()) {
            if (Conf_Item.at(Test.Finshed_Count).toInt() == 13) {
                ...
                if (NOload_Up_Finsh) {
                    ...
                    Ready_Stop_NOLOAD = true;
                }
            } else if (Conf_Item.at(Test.Finshed_Count).toInt() == 14) {
                if (BEMF_Up_Finsh) {
                    //
                } else {
                    Ready_Stop_BEMF = true;
                }
            } else if (Conf_Item.at(Test.Finshed_Count).toInt() == 12) {
                Test.Interrupted = true;
                if (LOAD_Up_Finsh) {
    ...
    case 16:
        ...
        break;
    case 17:
        PG_Fre = list.at(0).toDouble()/1000;  qDebug() << "2018-3-8" << PG_Fre;
        PG_Bb = list.at(1).toDouble()/1000;
        PG_High = list.at(2).toInt();
        PG_Low = list.at(3).toInt();
        break;
    case 18:
        Self_starting = list;
        break;
    ...
    case 22:
        ...
        break;
    case 23:
        qDebug() << "2323" << Conf_Item.at(Test.Finshed_Count).toInt();
        if (Conf_Item.at(Test.Finshed_Count).toInt() == 13) { // 空载
            NOload_Up_Finsh = false;
            frame.can_id = 0x2C;  frame.can_dlc = 0x03; // Test-----------PG Hall A
            frame.data[0] = 0x09; // 读取频率　占空比
            frame.data[1] = 0x01;
            ...
            Load_Update = list;
            Hall_A_ing = false;
        } else if (Conf_Item.at(Test.Finshed_Count).toInt() == 14) { // 反电动势
            BEMF_Up_Finsh = false;
            Can_Stop(0x2C);
            ...
            emit WriteMessage(6000, 0x77, QString("").toUtf8());
            for (j = 0; j < 1200; j++) { // 负载时间
                usleep(1000);
            }
            Can_Stop(0x2C);
            Load_Update = list;
...
void w_Test::Test_Overtime_Alarm(int Bug_Count)
{
    ...
    if ((Bug_Count) > 100) {
        ...
        State.IR  = true;
        if (False_Test) {
            Test_WMessage("  功率板未能测试到反电动势，中止测试  ");
        } else {
            //
        }
    } else {
...
* @retval 	None
***********************************************************/
void w_Test::Test_Mag_MagDir(int mag_rotation)
{
    uint16_t i = 0, j = 0;
    int16_t  temp = 0;
...
{
    int i, j, PWR_Data; qDebug() << "Test_Delete_Row----------" << value << Test.Status_Ing;
    int PG_True;
    switch (value) {
    case 1: // -电阻
        if ((DLR_UpDown.at(20).toDouble() != 0.0) && (DCR_Total_Number >= 3)) { // -包含电阻不平衡度
    ...
    case 7: // -功率
        ...
        if ((Ini_Motor == QString(tr("PG"))) && (strPWR.at(5+PWR_Data).toInt() == 2)) { // -包含PG
            ...
            strJudge  = strJudge.mid(0, strJudge.size() - 1);
            StrJudge_Size  = strJudge.size();
            PG_Result_List = PG_Result_List.mid(PG_Result_List.size() - 1);
            Test.Current_Row = Test.Current_Row - 1;
            for (j = 0; j < 400; j++) {
...
void w_Test::Pub_MAG_Param(QStringList mag_data, int *waveform_point)
{
    ...
    if (strOPP.at(6).toInt() != 2) { // -判断磁旋不是 不测
        if ((mag_back_number == 1) || (mag_back_number == 0)) {
            for (i = 0; i < 400; i++) {
        ...
        if (mag_back_number == 1) {
            if (((strOPP.at(NUM_OPP*0+3).toInt()) == 2) && \
                ...
                Test_Mag_MagDir(mag_set_rotation); // -测试磁旋
            }
        } else {
            //
        }
...
/******************************************************************************
  ...
  * brief:      PWR-的参数
******************************************************************************/
void w_Test::Pub_PWR_Param(QStringList pg_data, int *waveform_point)
{
    ...
    Test_PWR_Widget(Turn_Elec_force, 2);
    PG_Param(pg_data, waveform_point);
...
/******************************************************************************
  ...
  * brief:      PG-的参数
******************************************************************************/
void w_Test::PG_Param(QStringList pg_data, int *waveform_point)
{
    int i;
