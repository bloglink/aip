...
QStringList Widget_DLR::DCR_Test_Result(can_frame canframe,  int Back_out_station)
{
    ...
    double dlr_data; dlr_data = Back_QStringList.at(0).toDouble();
    if ((Back_out_station == 0x13)) {  // 线路补偿 (DLR_Compensate_Left) &&
    ...
    if (DCR_Offect_Station == tr("Left")) {
        for (i = 0; i < 8; i++) {
            ...
            if ((strRES_Compensate.toDouble()) < \
                    (2*(qPow(10, (strRES_Compensate_Grade) - 2)))) { // *10/100
                // 电阻补偿5% 9月7日升级10%  百分之十
...
QStringList Widget_DLR::DCR_Test_Param(int DCR_Balance)
{
    QString str;
    QStringList  strSql;
    ...
    if (DCR_Balance == 1) {
        if ((Copy_DLR_List.at(7) != "0.0") && (DLR_TestNumber >= 3)) {
            strTest.append(QString(tr("电阻")));
            strParam.append(QString(tr("不平衡度")) + Copy_DLR_List.at(7) + "%");
            strSql.append(tr("电阻"));
        }   //-将(-电阻-)修改为(电阻)
    } else if (DCR_Balance == 2) {
        if ((Copy_DLR_List.at(7) != "0.0") && (DLR_TestNumber >= 2)) {
            strTest.append(QString(tr("电阻")));
            strParam.append(QString(tr("不平衡度")) + Copy_DLR_List.at(7) + "%");
            strSql.append(tr("电阻"));
        }   //-将(-电阻-)修改为(电阻)
    } else if (DCR_Balance == 3) {
        if ((Copy_DLR_List.at(7) != "0.0") && (DLR_TestNumber >= 3)) {
            strTest.append(QString(tr("电阻")));
            strParam.append(QString(tr("不平衡度")) + Copy_DLR_List.at(7) + "%");
            strSql.append(tr("电阻"));
