...
void Widget_IMP::Pri_IMP_Init(QString File)
{
    ...
    ui->ittTab->setColumnWidth(3, 52); // 电晕
    ui->ittTab->setColumnWidth(4, 40); // 次数
    ui->ittTab->setColumnWidth(5, 40); // 面积
    ui->ittTab->setColumnWidth(6, 40); // 差积
    ui->ittTab->setColumnWidth(7, 52); // 电晕
    ...
    if (!(set_File.contains("IMP/righvalue"))) { // 17 设置
        QStringList impvalue = set_File.value("IMP/value").toStringList();
        set_File.setValue("IMP/righvalue", impvalue);
    } else {
        //
    }
 ...
 *  @retval:
*******************************************************************************/
int  Widget_IMP::Pri_IMP_Area(int *stwave, int *wave, uint8_t origin, uint16_t terminal)
{
    uint32_t starea = 0;
    uint32_t area = 0;
    int32_t  diff;
    ...
    diff = (area - starea)*100;
    diff = (diff/((int32_t)starea));
    if (diff < (-99)) {
        diff = -99;
    } else if (diff > (99)) {
        diff = 99;
 ...
 * @retval:
*******************************************************************************/
int  Widget_IMP::Pri_IMP_Diff(int *stwave, int *wave, uint8_t origin, uint16_t terminal)
{
    uint32_t starea = 0;
    uint32_t diff = 0;
    uint32_t diff2 = 0;
...
void Widget_IMP::on_ittTab_cellChanged(int row,  int column)
{
    ...
    if ((column >= 3) && (column <= 8)) {
        ...
        if (column == 3) {
            if (ui->ittTab->item(row, column)->text().toInt() >= 5000) {
                ui->ittTab->item(row, column)->setText("5000");
...
void Widget_IMP::IMP_NetData(QStringList list, QString str) {
    ...
    if (str == QString(tr("test"))) {
        ...
        for (i = 0; i < 8; i++) {
            Copy_IMP_List.replace(i*NUM_ITT+17, list.at(i));
        }
...
QStringList Widget_IMP::Pub_Main_IMP_Result(int ittwaveframe, int *waveItt, int station)
{
    ...
    if (station == 0x13) {
        ...
        imp_result.replace(2, QString::number(Pri_IMP_Flut(waveItt, IMP_origin, IMP_terminal)));
        imp_result.replace(3, QString::number\
                           (Pri_IMP_Phase(IMP_Right_Wave[11]->sample_point[ittwaveframe], \
                            waveItt, IMP_origin, IMP_terminal)*100/(100+Area)));
    } else {
        //
    }
...
QStringList Widget_IMP::Pub_Main_IMP_Result_Compare(int ittwaveframe, \
    ...
    imp_result.replace(1, QString::number(Pri_IMP_Diff(Compare_waveItt, waveItt, \
                                                       IMP_origin, IMP_terminal)));
    imp_result.replace(2, QString::number(Pri_IMP_Flut(waveItt, IMP_origin, IMP_terminal)));
    imp_result.replace(3, QString::number(Pri_IMP_Phase(Compare_waveItt, \
                                  waveItt, IMP_origin, IMP_terminal)*100/(100+Area)));
    ...
    if (up_down) {
        for (j = 0; j < 8; j++) {
            if ((Copy_IMP_List[j*NUM_ITT + 8].toInt())) {
                area_down.replace(j, Copy_IMP_List.at(j*NUM_ITT + 17) + "~");
